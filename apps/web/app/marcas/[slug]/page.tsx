...
export default async function Brand({params}:{params:Promise<{slug:string}>}){
 ...
 const manufacturer=await apiOrNull(`/public/manufacturers/${encodeURIComponent(slug)}`);
 if(!manufacturer)notFound();
 const products=await api(`/public/products?manufacturer=${encodeURIComponent(slug)}`);
 return <><SiteHeader/><main className="shell discoveryPage">
  ...
  <section className="grid catalogProducts">{(products.items??[]).map((p:any)=><ProductCard key={p.id} p={p}/>)}</section>
 </main></>;
}
